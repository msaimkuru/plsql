 ...
 * But wait, that's the sort of error you can with a SELECT-INTO that returns 
 * more than one row. Why is it showing up here?
 * 
...
BEGIN
   ...
   LOOP
      l_cnt := l_cnt + 1;
      DBMS_OUTPUT.PUT_LINE (LPAD(l_cnt, 3, ' ')||') '||l_ids(indx));
   END LOOP;
   --
   ROLLBACK;
END
