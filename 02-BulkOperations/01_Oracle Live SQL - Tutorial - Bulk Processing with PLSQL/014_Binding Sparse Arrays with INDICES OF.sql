 ...
 * over any undefined index values." Note that the collection referenced in the 
 * INDICES OF clause must be indexed by PLS_INTEGER; you cannot use 
 * string-indexed collections. But the datatype of the elements in the 
 * collection can be anything - they are not referenced at all with INDICES OF.
 */
 ...
 */
DECLARE  
   TYPE employee_aat IS TABLE OF saimk.employees.employee_id%TYPE 
...
/*
 * Here is a more interesting use of INDICES OF: We populate a second collection 
 * to serve as an "index" into the bind array. We also include the BETWEEN 
 * clause, which can be used to further restrict the set of elements in the 
 * indexing collection that will be used to bind values into the DML statement. 
 ...
 * will not.
 * 
 * Not good. And that's why I suggest you use the syntax that is appropriate to 
...
DECLARE 
  ...
  /* 
   * 'DEFAULT' version doesn't assume the first defined index value is 1,
   * and it handles empty collections just fine, but it does assume the 
   * collection is densely filled. 
   *
   * 'INDICESOF' ersion handles all.
   */
  --
  ...
  BEGIN
    BEGIN
      ...
      EXCEPTION
        WHEN OTHERS THEN
          dbms_output.put_line ('Dense:'||SQLERRM);
          --
          ROLLBACK;
    END;
    --
    BEGIN
      /* But not OK when it is sparse. */
