 ...
 * To see this happen, run the code below (note: varchar2a is a collection type 
 * of strings defined in the DBMS_SQL package).
 */
 ...
 * Oracle Database offers a LIMIT clause for BULK COLLECT. Indeed, when using 
 * BULK COLLECT we recommend that you never or at least rarely use an 
 * "unlimited" BULK COLLECT which is what you get with a 
 ...
 * I demonstrate this issue below, but first, a reminder: there are 107 rows in 
 * the employees table.
 *
 ...
 * Wait, what? Is that right? Do we see "Total rows fetched: 100"? Yes, we do. And 
 * therein lies the trap. You cannot continue to use the same EXIT WHEN 
...
BEGIN
  ...
  LOOP
    ...
    FOR indx IN 1..l_emps.COUNT LOOP
      l_cnt := l_cnt + 1;
      dbms_output.put_line(LPAD(l_cnt, 3, ' ')||')'||l_emps(indx).first_name||', '||l_emps(indx).last_name||', '||l_emps(indx).salary);
    END LOOP;     
  END LOOP;
  --
  CLOSE c_emps; 
